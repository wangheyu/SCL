...
+------------*/
 ...
 fprintf(out, "%6d  %6d  %6d **(Nnd, Nee, Nsd)**\n", r_Nn, r_Ne, r_Ns);
 for(n=0; n<r_Nn; n++)
   fprintf(out, "%6d  %18.15e %18.15e %6d\n",
...
+---------------*/
 ...
 fprintf(out, "%6d  %6d  %6d **(Nee, Nnd, Nsd)**\n", r_Ne, r_Nn, r_Ns);
 for(e=0; e<r_Ne; e++)
   fprintf(out, "%6d  %6d %6d %6d  %6d %6d %6d %6d %6d %6d\n",
...
+------------*/
 ...
 fprintf(out, "%d\n", r_Ns);
 for(s=0; s<r_Ns; s++)
   fprintf(out, "%6d  %6d %6d %6d %6d %6d \n", s, r_side[s].c,
       r_side[s].d, r_side[s].ea, r_side[s].eb, r_side[s].mark);
 fprintf(out, "--------------------------------\n");
 fprintf(out, "   s:    c    d   ea   eb\n");
...
/*------------+
|             |
|Tecplot data (added by Changqing Hu)
|             |
+------------*/
 ...
 fprintf(out, "ZONE I=%d, J=%d, F=FEPOINT\n", r_Nn, r_Ne);
 for(n=0; n<r_Nn; n++)
   fprintf(out, "%18.15e %18.15e\n",
		 r_node[n].x, r_node[n].y);
 for(e=0; e<r_Ne; e++)
   fprintf(out, "%6d %6d %6d  %6d \n",
	  r_elem[e].i+1,  r_elem[e].j+1, r_elem[e].k+1, r_elem[e].k+1);
 fclose(out);
 */
